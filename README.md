...
An event-driven microservices platform for fitness tracking and AI-driven workout recommendations. Users record workout activities, and an asynchronous processing pipeline uses Google Gemini to generate personalized performance analysis, recovery guidance, training progression tips, and safety considerations.
...
- [Infrastructure and Supporting Services](#infrastructure-and-supporting-services)
...
- [Getting Started and Local Setup](#getting-started-and-local-setup)
  ...
  - [2. Hybrid Development (Recommended)](#2-hybrid-development-recommended)
  - [3. Full Stack in Docker](#3-full-stack-in-docker)
  - [4. Production Deployment](#4-production-deployment)
- [Spring Profiles and Configuration Hierarchy](#spring-profiles-and-configuration-hierarchy)
...
│  - Eureka Discovery Server (Port: 8761): Dynamic registry and client-side load balancing              │
...
Retrieves all recorded workout sessions for the user specified in `X-User-ID`.
...
Follow this walkthrough to test the entire ecosystem end-to-end:
...
Under the hood:
1. The Gateway validates the JWT token.
...
- **Java 21 JDK** (Eclipse Temurin, OpenJDK, or Homebrew)
...
Run databases, message broker, and discovery infrastructure in Docker, while running the Spring Boot microservices directly on your machine / IDE for fast debugging and development.

#### Step 1: Start Infrastructure in Docker
```bash
docker compose --env-file .env.dev -f docker-compose.dev.yml up -d
```
Starts: PostgreSQL, MongoDB, RabbitMQ, Eureka Server, Config Server, Keycloak, and Gateway.
...
Run the stack with restricted host bindings (`127.0.0.1`), memory constraints, and production configuration profiles:
...
## Spring Profiles and Configuration Hierarchy

| Profile | Purpose | DB and Service Resolution | Typical Host |
|---|---|---|---|
...
3. Properties are injected into the Spring Environment.
...
For details on each individual microservice, refer to:

- [API Gateway Service Documentation](./gatewayservice/README.md)
- [Config Server Documentation](./configserver/README.md)
